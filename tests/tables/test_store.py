...
from af.tables.update import CDCInputs, TablesSettings, make_step, update

from .conftest import write_rules
from .test_cdc import row, write_tsv


def settings(tmp_path: Path, rows: list[dict[str, str]]) -> TablesSettings:
    store_root = tmp_path / "store"
    if not store_root.exists():
        Store.create(store_root)
    return TablesSettings(
        rules=write_rules(tmp_path / "rules", all_optional=True),
        store=store_root,
        report=tmp_path / "state" / "report.txt",
        published=tmp_path / "state" / "published.json",
...
def events(s: TablesSettings) -> dict[str, str]:
    data = json.loads(s.published.read_text())
...
def test_errors_publish_nothing(tmp_path):
    ...
    assert Store.open(s.store).list_datasets(KIND) == []
...
def test_stored_table_reads_back_with_its_hash(tmp_path):
    ...
    (fresh,) = cdc.read(s.cdc.tsv, Rules(s.rules)).tables
...
def test_pipeline_step_names_its_inputs_by_role(tmp_path):
    ...
    parameters = {
        ...
        "cdc": {"tsv": str(s.cdc.tsv)},
    }
    step = make_step(parameters, base_dir=tmp_path)
    assert step.name == "tables-update" and set(step.inputs) == {"rules", "cdc_tsv"}
    assert step.outputs[0].path == tmp_path / "state" / "published.json"
